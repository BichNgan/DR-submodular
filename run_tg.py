...
from algorithms import ThresholdGreedy 
from result import *
from configs import *
...
if __name__ == "__main__":
    ...
    for dataset, delimiter in zip(datasets, delimiters):
        ...
        for b, k in zip(b_values, k_values):
            ...
            B = np.full(n, b)
            start = time.time()
            tg = ThresholdGreedy(e_arr=E, b_arr=B, f=f, k=k, epsilon=epsilon)
            x = tg.run()
            ...
            save_result(df, f'{output_dir}/tg_{dataset}.csv')
